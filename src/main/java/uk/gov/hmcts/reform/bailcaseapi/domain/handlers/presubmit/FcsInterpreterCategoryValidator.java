...
public class FcsInterpreterCategoryValidator implements PreSubmitCallbackHandler<BailCase> {
    ...
    public PreSubmitCallbackResponse<BailCase> handle(
        ...
        if (bailCase.read(FCS_INTERPRETER_YESNO, YesOrNo.class).orElse(YesOrNo.NO) == YesOrNo.YES) {
            ...
            boolean has4Fcs = bailCase.read(HAS_FINANCIAL_COND_SUPPORTER_4, YesOrNo.class).orElse(YesOrNo.NO) == YesOrNo.YES;
            boolean has3Fcs = bailCase.read(HAS_FINANCIAL_COND_SUPPORTER_3, YesOrNo.class).orElse(YesOrNo.NO) == YesOrNo.YES;
            boolean has2Fcs = bailCase.read(HAS_FINANCIAL_COND_SUPPORTER_2, YesOrNo.class).orElse(YesOrNo.NO) == YesOrNo.YES;
            boolean hasFcs = bailCase.read(HAS_FINANCIAL_COND_SUPPORTER, YesOrNo.class).orElse(YesOrNo.NO) == YesOrNo.YES;
